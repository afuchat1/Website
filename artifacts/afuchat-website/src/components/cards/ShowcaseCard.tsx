    ...
    >
      ...
      <div className="flex flex-1 flex-col px-2 pb-2 pt-5 sm:px-3 sm:pb-3">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3">
          {hrefExternal ? (
            <a href={href} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-[#2b5ea4]">
              {hrefLabel}
            </a>
          ) : (
            <Link href={href} className="text-xs font-semibold text-[#2b5ea4]">
              {hrefLabel}
            </Link>
          )}
          {secondaryHref && secondaryLabel && (
            <a href={secondaryHref} target="_blank" rel="noopener noreferrer" className="text-xs text-[#6f89a7] hover:text-[#2b5ea4]">
              {secondaryLabel}
            </a>
          )}
        </div>
