        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center py-20">
          <div>
            <motion.p
              ...
              className="text-blue-400 font-semibold text-xs uppercase tracking-widest mb-5"
            >
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {['#6366F1','#10B981','#F59E0B','#EF4444'].map((c, i) => (
                  <div key={i} className="w-7 h-7 rounded-full border-2 border-[#040c1e]" style={{ background: c }} />
                ))}
              </div>
              <span className="text-white/40 text-sm">Trusted by millions globally</span>
            </motion.div>
          </div>
          <motion.div
