...
export default function Developers() {
  return <div className="studio-shell">
    <section className="max-container studio-section grid items-center gap-12 lg:grid-cols-[1fr_.82fr] lg:gap-24">
      ...
      <div className="border border-white/10 bg-[#0b1b31]/50 p-6"><img src={illSecDeveloper} alt="AfuChat developer platform" className="w-full" /><div className="mt-5 border-t border-white/10 pt-4 text-[10px] text-[#5d7694] studio-mono">BUILD / SHIP / SCALE</div></div>
    </section>
    <section className="border-y border-white/[.08] bg-[#081529]/55"><div className="max-container studio-section"><div className="mb-10 flex items-end justify-between gap-6"><div><p className="studio-kicker mb-5 text-[#8be7c4]">Documentation</p><h2 className="text-3xl font-semibold text-[#e6f1ff] sm:text-4xl">A clear path from idea<br />to production.</h2></div><span className="hidden text-xs text-[#5d7694] studio-mono sm:block">API SURFACE / 06</span></div><div className="grid border-t border-white/10 md:grid-cols-2 lg:grid-cols-3">{resources.map(({ icon: Icon, label, desc, color }, index) => <motion.div key={label} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * .05 }} className="border-b border-white/10 p-6 md:border-l md:first:border-l-0"><Icon className="h-5 w-5" style={{ color }} strokeWidth={1.5} /><h3 className="mt-9 text-sm font-semibold text-[#e6f1ff]">{label}</h3><p className="mt-3 text-sm leading-relaxed text-[#6f89a7]">{desc}</p></motion.div>)}</div></div></section>
    <DeveloperSection />
    <Footer />
  </div>;
}