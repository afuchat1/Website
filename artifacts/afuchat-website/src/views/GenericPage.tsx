  ...
  const isLegal = pageType === 'privacy' || pageType === 'terms';
   return <div className="studio-shell atmosphere-page">
    {isLegal ? <LegalDocument title={title} type={pageType} /> : <>
        <section className="page-hero max-container studio-section grid items-center gap-12 lg:grid-cols-[1fr_.76fr] lg:gap-24"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}><div className="accent-bar mb-7" /><p className="studio-kicker mb-6" style={{ color: content.accent }}>AfuChat / {type}</p><h1 className="max-w-3xl text-[clamp(3.5rem,8vw,7.5rem)] font-semibold leading-[.88] tracking-[-.075em] text-[#e6f1ff]">{title}</h1><p className="mt-8 max-w-xl text-lg leading-relaxed text-[#91a8c4]">{content.intro}</p></motion.div>{content.illustration && <motion.div initial={{ opacity: 0, scale: .94 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .1 }} className="page-hero-art flex justify-center"><img src={content.illustration} alt={title} className="w-full" /></motion.div>}</section>
       <section className="atmosphere-band"><div className="max-container studio-section grid gap-12 lg:grid-cols-[.42fr_1fr] lg:gap-24"><div><p className="studio-kicker" style={{ color: content.accent }}>The brief</p><p className="scroll-line mt-6 max-w-[14rem] text-sm leading-relaxed text-[#6f89a7]">A clear point of view, useful systems, and the patience to make the details work.</p></div><div>{content.sections.map((section, index) => <ScrollReveal key={section.heading} delay={index * .06} className="grid gap-4 py-8 sm:grid-cols-[.34fr_1fr]"><h2 className="text-xl font-semibold text-[#e6f1ff]">{section.heading}</h2><p className="text-sm leading-relaxed text-[#91a8c4]">{section.body}</p></ScrollReveal>)}</div></div></section>
       <section className="max-container studio-section-tight"><div className="mb-8 flex items-end justify-between gap-5"><div><p className="studio-kicker mb-4" style={{ color: content.accent }}>Product system</p><h2 className="text-3xl font-semibold text-[#e6f1ff]">Explore the ecosystem.</h2></div><Link href="/products" className="studio-link flex items-center gap-2 text-xs">All products <ArrowUpRight className="h-3.5 w-3.5" /></Link></div><div className="color-panel grid overflow-hidden rounded-[1.6rem] sm:grid-cols-2 lg:grid-cols-4">{PRODUCT_DATA.slice(0, 4).map((product, index) => { const Icon = product.icon; return <ScrollReveal key={product.id} delay={index * .06}><Link href={product.path} className="group flex items-center gap-3 border-b border-[#123b7a]/10 px-5 py-5 sm:border-l first:sm:border-l-0"><Icon className="h-5 w-5" style={{ color: product.color }} strokeWidth={1.5} /><span className="text-sm text-[#4d6884] group-hover:text-[#102f53]">{product.name}</span><ArrowUpRight className="ml-auto h-3.5 w-3.5 text-[#6f89a7] group-hover:text-[#165fbd]" /></Link></ScrollReveal>; })}</div></section>
    </>}
     <section className="max-container studio-section-tight"><div className="color-panel rounded-[1.6rem] flex flex-col items-start justify-between gap-7 p-8 sm:flex-row sm:items-center sm:p-10"><div><p className="studio-kicker mb-4" style={{ color: content.accent }}>Next step</p><h2 className="text-2xl font-semibold text-[#e6f1ff]">Need to talk through something?</h2></div><Link href="/contact" className="studio-button studio-button-ghost">Contact the studio <ArrowUpRight className="h-4 w-4" /></Link></div></section>
    <Footer />
  </div>;
}