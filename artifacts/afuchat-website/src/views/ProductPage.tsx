  ...
  return <div className="studio-shell">
    <section className="max-container studio-section grid items-center gap-12 lg:grid-cols-[.92fr_1.08fr] lg:gap-24">
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <a href="https://web.afuchat.com/register" className="studio-button mt-9 text-[#06101f]" style={{ background: product!.color }}>Get started free <ArrowUpRight className="h-4 w-4" /></a>
      </motion.div>
      <motion.div initial={{ opacity: 0, scale: .93 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: .1 }} className="relative border border-white/10 bg-[#0b1b31]/55 p-5 sm:p-10"><div className="mb-7 flex justify-between border-b border-white/10 pb-4 text-[10px] text-[#5d7694] studio-mono"><span>{product!.name.toUpperCase()} / PRODUCT VIEW</span><span>ACTIVE</span></div><img src={product!.illustration} alt={product!.name} className="w-full drop-shadow-2xl" /></motion.div>
    </section>
    <section className="border-y border-white/[.08] bg-[#081529]/55"><div className="max-container studio-section-tight grid items-center gap-12 lg:grid-cols-[.65fr_1fr] lg:gap-24"><div><p className="studio-kicker mb-5 text-[#9d8cff]">Ecosystem / Designed to connect</p><h2 className="text-3xl font-semibold leading-tight text-[#e6f1ff] sm:text-4xl">Works even better together.</h2><p className="mt-5 max-w-md text-sm leading-relaxed text-[#6f89a7]">AfuChat products stay independent while sharing a thoughtful foundation for the people and organizations using them.</p></div><img src={illSecEcosystem} alt="AfuChat ecosystem" loading="lazy" className="mx-auto w-full max-w-md" /></div></section>
